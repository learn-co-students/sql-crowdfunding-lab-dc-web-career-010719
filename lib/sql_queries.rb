...
"select projects.title, (sum(pledges.amount) - projects.funding_goal) as amount_over_goal from projects inner join pledges on projects.id = pledges.project_id group by projects.title having (sum(pledges.amount) - projects.funding_goal) >= 0;"
end
