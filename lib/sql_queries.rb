...
"SELECT projects.title, sum(pledges.amount )- projects.funding_goal AS over FROM projects JOIN pledges ON pledges.project_id == projects.id GROUP BY projects.title HAVING over >= 0"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(pledges.amount) AS am FROM users JOIN pledges ON  users.id == pledges.user_id group by users.name ORDER By am"
end
